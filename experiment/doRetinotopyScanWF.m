function doRetinotopyScanWF(params)
...
uPad = params.padUpperPx;
lPad = params.padLowerPx;
nPad = 960-uPad-lPad;
sPad = round((1280-nPad)/2);
params.display.fixY = params.display.fixY+uPad;
params.display.numPixels = [1280 960];

% make/load stimulus
stimulus = retLoadStimulus(params);
stimSize = size(stimulus.images{1});
stimulusShifted = uint8(zeros(960,1280,stimSize(3),stimSize(4)));
stimulusShifted(:,:,:,:) = 128;
stimulusShifted(uPad+1:960-lPad,sPad+1:sPad+nPad,:,:) = stimulus.images{1}(:,:,:,:);
stimulus.images{1} = stimulusShifted;
